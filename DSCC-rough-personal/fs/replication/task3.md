...
**Primary-backup** is simpler, correct for this workload, and matches the spec's hint to "stick to a simple approach."
...
- The existing `fs/fs.proto` is **unchanged**. Task 3 adds only `replication.proto`.
...
- When it restarts, it uses `SyncState` to catch up (see §6.5).
