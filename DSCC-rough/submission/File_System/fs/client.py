...
def run_exhaustive_tests():
    ...
    f_lock = f"lock_{run_id}.txt"
    f_unlock = f"unlock_{run_id}.txt"
    ...
    try:
        ...
        expect_exception("Opening non-existent file blocked", lambda: client.open(f"does_not_exist_{run_id}.txt", mode="r"))
        ...
        # TEST 16
        print_header("TEST 16: File Lock Blocks Writes Until Timeout")
        client.create(f_lock)
        h = client.open(f_lock, mode="w")
        client.write(h, 0, b"lock-protected-data")
        client.lock_file(f_lock, timeout_seconds=2)

        expect_exception("Close/write-back blocked while lock is active", lambda: client.close(h))

        notifications = client.wait_for_lock_notification(timeout_seconds=5)
        if notifications and "timed out" in notifications[0]:
            ok(f"Received lock-timeout notification: {notifications[0]}")
        else:
            fail(f"Expected lock-timeout notification, got: {notifications}")

        client.close(h)
        h = client.open(f_lock, mode="r")
        locked_data = client.read(h, 0, 64)
        client.close(h)
        if locked_data == b"lock-protected-data":
            ok("Write succeeded after timeout-based automatic unlock")
        else:
            fail(f"Unexpected data after unlock: {locked_data}")

        # TEST 17
        print_header("TEST 17: Explicit Unlock Releases Lock Immediately")
        client.create(f_unlock)
        h = client.open(f_unlock, mode="w")
        client.write(h, 0, b"manual-unlock-data")
        client.lock_file(f_unlock, timeout_seconds=15)
        client.unlock_file(f_unlock)
        client.close(h)

        messages = client.get_lock_notifications()
        if any("released explicitly" in msg for msg in messages):
            ok("Received explicit unlock notification")
        else:
            fail(f"Expected explicit unlock notification, got: {messages}")

        h = client.open(f_unlock, mode="r")
        unlocked_data = client.read(h, 0, 64)
        client.close(h)
        if unlocked_data == b"manual-unlock-data":
            ok("Explicit unlock allowed immediate write-back")
        else:
            fail(f"Unexpected data after explicit unlock: {unlocked_data}")
