...
Implements leader election, log replication, and snapshotting. This module is pure Raft logic;
the gRPC plumbing lives in raft_server.py.
"""
...
class RaftNode:
    ...
    Public interface used by raft_server.py:
        append_entry(op_type, filename, data, client_id, seq_num)
            → blocks until committed, returns {"new_version": int} | raises
        handle_request_vote(args)  → reply dict
        handle_append_entries(args) → reply dict
        handle_install_snapshot(args) → reply dict
        get_leader_info() → (leader_id, leader_address)
        is_leader() → bool
    """
    ...
    def __init__(self, node_id: int, peers: dict, persist_dir: str,
                 apply_fn=None):
        """
        node_id    : int — unique id for this node (0, 1, 2, …)
        peers      : dict {node_id: "host:port"}  (excludes self)
        persist_dir: directory to store durable state
        apply_fn   : callable(entry_dict) → result  — applies a committed
                     log entry to the application state machine.
                     Called with self.lock NOT held.
        """
    ...
    def append_entry(self, op_type: str, filename: str, data: bytes,
        ...
        """
        Propose a new entry to the Raft log (leader only).
        Blocks until the entry is committed and applied, or raises on error.
        Returns the result dict from apply_fn.
        """
        with self.lock:
        ...
        logger.info(f"[{self.node_id}] Appended entry idx={idx} op={op_type} "
                    f"file={filename}")
    ...
    def handle_append_entries(self, args: dict) -> dict:
        """§5.3 — AppendEntries RPC handler (also heartbeat)."""
        with self.lock:
            term        = args["term"]
    ...
    def handle_install_snapshot(self, args: dict) -> dict:
        """Install a snapshot from the leader (used during catch-up)."""
        with self.lock:
            term = args["term"]
            if term > self.current_term:
                self._become_follower(term)
    ...
    def _start_heartbeat(self):
        """Start periodic heartbeat thread (leader only)."""
        def hb_loop():
            while True:
                with self.lock:
                    if self.state != self.LEADER:
    ...
    def _send_append_entries_all(self):
        """Send AppendEntries (or InstallSnapshot) to every peer in parallel."""
        for pid, stub in self.peer_stubs.items():
    ...
    def _candidate_log_up_to_date(self, cand_last_idx: int,
                                   cand_last_term: int) -> bool:
        """§5.4.1 — Is candidate's log at least as up-to-date as ours?"""
                ...
                for idx, entry in to_apply:
                    ...
                    if entry["op_type"] != "noop" and self.apply_fn:
                        try:
                            ...
                            entry["data"] = (bytes.fromhex(entry["data"])
                                             if entry.get("data") else b"")
                            result = self.apply_fn(entry)
                        except Exception as e:
                            logger.error(f"[{self.node_id}] Apply failed "
                                         f"idx={idx}: {e}")
                            result = e
    ...
    def _persist_log(self):
        """Persist the Raft log (called with lock held)."""
